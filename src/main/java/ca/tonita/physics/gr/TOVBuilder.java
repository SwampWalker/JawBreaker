...
public class TOVBuilder {
     ...
     *
     * @param tov a <code>TOVData</code> object to store the result
     * @param rk4Guess a <code>TOVData</code> generated by RK4 to use as the guess
     * @param eos the equation of state to use
     * @param basis the <code>LinearlyMappedBasis</code> to use for the spectral
     * solution
     */
    public static void spectralGuess(TOVData tov, TOVData rk4Guess, TabulatedHermite eos, LinearlyMappedBasis basis) {
        ...
        basis.setRight(rk4Guess.getRadius());
        double[] spectralRadii = basis.getAbscissas();
        ArrayList<Double> radii = rk4Guess.getRadii();
        ArrayList<double[]> newPoints = new ArrayList<double[]>();

        // First point is the same.
        newPoints.add(new double[]{rk4Guess.getPressure(0), 0, 0});
        ...
        for (int i = 1; i < spectralRadii.length - 1; i++) {
            ...
            double[] newPoint = RK4.step(rk4Guess.getVariables(iRadii), radii.get(iRadii), eqns, Math.abs(radii.get(iRadii) - spectralRadii[i]));
            newPoints.add(newPoint);
        }
        // Last point we read off.
        newPoints.add(rk4Guess.getVariables(radii.size() - 1));
